...
namespace by {
     ...
     *  @details Abbreviation for "dynamic loading for library". Handles
     * platform-independent dynamic loading. Can load libraries into memory and find
     * functions to convert them into function pointers.
     *
     ...
     *  Uses @ref tmay, so it's recommended to familiarize yourself with tmay
     * beforehand.
     *
     *
     *  @remark RAII idiom
     *  dlib is implemented using RAII idiom. When the instance is destroyed,
     * function pointers returned externally become unusable.
     *
     *  @section usage Usage
     *  Example showing the complete process of loading a dynamic library and
     * calling a function. Note that `rel()` explicitly releases resources. When an
     * error occurs, the comma operator `(rel(), false)` is used to clean up the
     * dlib object before returning:
     *  @code
     ...
     *               .ret((rel(), false));
     *      // Release resources with rel() first, then return false via comma
     * operator
     *
     *      typedef void (*entrypointFunc)(bicontainable*);
     *      constexpr const nchar* ENTRYPOINT_NAME = "byeol_bridge_cpp_entrypoint";
     *      auto info = lib.accessFunc<entrypointFunc>(ENTRYPOINT_NAME); // Result
     * as tmay WHEN(!info.has()) // Checking result with tmay's has() .err("couldn't
     * access entrypoint of %s pod: %d", path, info.getErr()) .ret((rel(), false));
     *
     *      (*info)(&tray); // If function is successfully retrieved, it can be
     * called
     *
    ...
    public:
         ...
         * @return tres containing function pointer on success, or error message on
         * failure
         * @note Library must be loaded first. Uses GetProcAddress on Windows, dlsym
         * on POSIX. Automatically calls rel() on failure.
         */
