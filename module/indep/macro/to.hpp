...
namespace by {
     ...
     *  @details `TO` supports the safe navigation features of modern languages very
     * intuitively and naturally. the basic usage is `<expression>
     * TO(yourAccessor())`. let's explain with an example first before explain
     * further.
     *
     *  usage:
     ...
     *      in language spec of c++, there is no sort of safe navigation thing, so
     ...
     *      @endcode
     *
     *      of course, this example illustrates a rather extreme train wreck
     * pattern, and is a design that should be avoided, but situations where you
     * need to access a pointer to a certain number of pointers occur frequently,
     * and if you don't always check in advance whether the pointer is valid every
     * time you dereference it, UB will occur.
     *
     *      to be:
     *          with safe navigation, whether you will receive nullptr or not is
     * determined after the dereference chain of all pointers is finished. so the
     * resulting code can become very concise.
     *          @code
     *              int getBrushColorCode(Resource r) {
     ...
     *          @endcode
     *
     *  @remark as you can see, it looks easy to use, but there are a few things
     * that you should be aware of.
     *      1. the previously mentioned `<expression>` does not simply mean pointers
     * and references, but also includes classes that satisfy the following
     * conditions.
     *
     ...
     * are usually smart pointers like `unique_ptr`. the byeol repository provides
     * separate smart pointers and classes that replace std::optional<T> for API
     * consistency and safe type checking. e.g. tstr<T>, tweak<T>, binder, tmay<T>,
     * tres<T> are included here.
     *
     *      the TO() macro is designed to work properly even if the return value of
     * the function is a value or reference to the above class. it works even if it
     * exists in the middle of safe navigation chain. for example,
     *
     *      @code
     *          struct Resource {
     *              Pallete* getPallete(); // this can return nullptr.
     ...
     * reference type. so, int* code = r TO(getPallete().getCanvas())
     *              TO(getBrush(BrushType.SYSTEM)) TO(getColorCode());
     *
     *              if(!code) return -1;
     *              return code;
     ...
     *      @endcode
     *
     *      2. if nullptr returned during the chain, the final result value becomes
     * the nullptr of last type of the chain. if it is T*, it will be nullptr, but
     * if it is T, i.e. a function that returns by value, the return value will be
     * T{}.
     *
